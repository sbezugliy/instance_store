...
class Instantiator
  ...
  def alias_check(type, name, clone, alias_name)
    ...
    raise NilAliasNameError.new(type, name)
  end

  def caller_check(type, name)
    raise(NoModuleError, type) unless @i.keys.include?(type)
    raise(NoClassError.new(type, name)) unless @i[type].keys.include?(name)
  end
